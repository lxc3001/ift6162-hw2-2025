...
def run_part1(
    *,
    ...
    if algo not in {"reinforce", "ppo", "td3", "all"}:
    ...
    if algo in ("reinforce", "all"):
        ...
        cfg = ReinforceConfig(seed=seed, episodes=episodes, device=device)
        ...
        stats = evaluate_policy(env, policy, n_episodes=10, device=device)
        print("REINFORCE eval:")
        print(stats)
        if plot_trajectory:
            traj = _rollout_trajectory(env, policy, seed=0, device=device)
            _plot_trajectory(
    ...
    if algo in ("ppo", "all"):
        print(f"Training PPO for {steps} environment steps...")
        cfg = PPOConfig(seed=seed, total_steps=steps, device=device)
    ...
    if algo in ("td3", "all"):
        print(f"Training TD3 for {steps} environment steps...")
        cfg = TD3Config(seed=seed, total_steps=steps, device=device)
        ...
        stats = evaluate_policy(env, actor, n_episodes=10, device=device)
        print("TD3 eval:")
        print(stats)
        if plot_trajectory:
            traj = _rollout_trajectory(env, actor, seed=0, device=device)
            _plot_trajectory(
    ...
    if plot_trajectory:
        baseline_traj = _rollout_trajectory(env, baseline.get_action, seed=0, device=device)
        _plot_trajectory(
...
if __name__ == "__main__":
    ...
    run_part1(
        algo="all",  # "reinforce" | "ppo" | "td3" | "all"
        seed=0,
        episodes=300,  # REINFORCE
        steps=20_000,  # PPO/TD3 env steps
        device="cpu",
        outdir="models",
